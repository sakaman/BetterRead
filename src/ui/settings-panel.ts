import { copySettings, type BetterReadSettings, type ThemeId, type UiThemeId } from "../core/settings.ts";
...
const PANEL_HTML = `
  ...
  <section class="panel" role="dialog" aria-label="BetterRead 阅读设置" hidden>
    <header class="header">
      ...
      <div class="heading"><h2 class="title">BetterRead</h2><p class="subtitle">微信读书体验增强 · v${__BETTERREAD_VERSION__}</p></div>
...
export class SettingsPanel {
  ...
  private readonly onSchemeChange = () => this.updatePanelTheme();
  ...
  private onInput(event: Event): void {
    ...
    switch (input.name) {
      ...
      case "uiTheme": next.uiTheme = input.value as UiThemeId; break;
  ...
  private syncFields(): void {
    ...
    setValue("uiTheme", this.settings.uiTheme);
  ...
  private updatePanelTheme(): void {
    this.host.dataset.uiTheme = this.settings.uiTheme === "system"
